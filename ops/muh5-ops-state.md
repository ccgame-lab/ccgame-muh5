...
## ĐANG LÀM / CHỜ
- **SDK redesign - OverviewPane ĐANG PORT** (2026-06-27): đã port xong layout mới OverviewPane.vue theo Claude Design "MU SDK Overlay". Layout: trái (player+VÍ CỦA TÔI compact dot+CheckinCard), phải (TÀI SẢN hero numbers Tôm+BlockKC + LIVE + Missions + utility). Build OK (724ms 0 error). **CHƯA DEPLOY** - cần push+deploy.sh.
  - File thay đổi: `resources/sdk/src/components/OverviewPane.vue` + built assets
  - Còn port: RankingPane (podium design), DonatePane (cash-shop groups), visual cleanup minor
  - Nạp Tôm tab = FROZEN (payment)
